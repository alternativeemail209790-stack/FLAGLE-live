...
// free account at https://www.eulerstream.com — raises the free connection
// limit. The game still works without it on the small free tier.
...
io.on("connection", (socket) => {
  ...
  socket.on("connect-tiktok", async ({ username }) => {
    ...
    try {
      ...
      console.error("Failed to connect to TikTok LIVE:", err?.name, raw);
      ...
      if (err?.name === "UserOfflineError" || lower.includes("offline") || lower.includes("not found")) {
        ...
        friendly = "Hit a temporary rate limit reading TikTok chat. Wait ~30 seconds and try again (getting a free key from eulerstream.com and adding it as TIKTOK_SIGN_API_KEY on Render fixes this for good — see README).";
      } else if (lower.includes("captcha") || lower.includes("blocked") || lower.includes("forbidden") || lower.includes("403")) {
        friendly = "TikTok is blocking this connection attempt. Wait a minute and retry; if it keeps happening, add a free key from eulerstream.com as TIKTOK_SIGN_API_KEY on Render (see README).";
      } else {
        friendly = `Could not connect (${err?.name || "error"}: ${raw}). Make sure the username is typed exactly right and that you're already LIVE, then try again.`;
      }
